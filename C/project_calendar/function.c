...
void ADDestroy(AD * _DAptr)
{
	...
	if(!_DAptr)
	{
		...
		{
			for (i = _DAptr -> numOfMeeting; i ; --i)/* more efficient with i = _DAptr -> numOfmeeting et faire --i*/
			{
...
void ADsave(const AD* const _DAptr,FILE * _file)
{
	int i;
	if(checkSegmentationError((void *) _DAptr) == SEGMENTATION_FAULT)
	{
	...
	for ( i = 0; i < _DAptr -> numOfMeeting; i++)
	{
		fprintf(_file,"%.2f\t%.2f\t%d\n",_DAptr->arrayMeeting[i]-> begin,_DAptr->arrayMeeting[i]-> end,_DAptr->arrayMeeting[i]-> room);
	}
	printf("\n" );
}
...
void loadCalendar(AD* _DAptr,FILE* _file)
{
	float begin, end;
	int room;
	meeting_t * meeting = NULL;
	...
	while(fscanf(_file,"%f%f%d",&begin, &end, &room)!=EOF)
	{
		meeting = createMeeting(begin,end,room);
		InsertMeeting(_DAptr, meeting);
	}
}
