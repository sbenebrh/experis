...
public:
    ...
    virtual const Record&    Report(const Item& _item)  ; //throw exception
            void      ReturnBook(const Book& _book) const ; // throw exception
            void      BorrowBook(const Book& _book) const ; // throw exception
    protected:
    void updateRecord(const Borrower& _borr); // throw exception
    ~BookAdmin();
private:
