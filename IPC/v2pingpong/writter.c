...
static int checkLastWritter(int msqid)
{
	int msgLen;
	char msg[MAXBYTES] = {0};
	char num[10];
	...
	pmsg = (struct msgbuf*)msg;
	pmsg -> mtype = EXCL_TYPE;

	msgrcv(msqid, pmsg, MAXBYTES, pmsg -> mtype, IPC_NOWAIT);

	writers = atoi(pmsg -> mtext);
	if(writers == 1)
	{
		return 1;
	}
	writers--;
	sprintf(num, "%d", writers);
	strcpy(pmsg -> mtext,num);
	
	msgsnd(msqid,pmsg,MAXBYTES,IPC_NOWAIT);
	return 0;
...
static int sendFirstMessage(int msqid)
{
	int msgLen;
	char msg[MAXBYTES] = {0};
	char num[10];
	...
	return msgsnd(msqid,pmsg,MAXBYTES,IPC_NOWAIT);
}
static int run_message_writter(int argc, char* argv[])
{
	key_t key;
	...
	printf("%d\n",msqid );
	if(sendFirstMessage(msqid) == FAIL)
	{
		Flags_destroy(flags);
		return FAIL;
	}
